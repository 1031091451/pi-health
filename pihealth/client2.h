...
int client_send_warn(int sockfd, int port);
    ...
    while (fscanf(input, "%s", buffer) != EOF) {
        if(strcmp(buffer, "warning") == 0) {
            ...
            while (1) {
                ...
                DBG("client wait ACK to")
            }
        } else {
            a = send(sockfd, "203", 3, 0);
            if (a < 0) DBG("send error\n")
            else  DBG("send success\n")
        }
        ++i;
        memset(buffer, 0, 200);
        continue;
